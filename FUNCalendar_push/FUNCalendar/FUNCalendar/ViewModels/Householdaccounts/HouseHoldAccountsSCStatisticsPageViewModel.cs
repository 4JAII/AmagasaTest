...
namespace FUNCalendar.ViewModels
{
    public class HouseHoldAccountsSCStatisticsPageViewModel : BindableBase,INavigationAware,IDisposable
    {
        ...
        private string _currentSCategory;
        public string CurrentSCategory
        {
        ...
        public HouseHoldAccountsSCStatisticsPageViewModel(IHouseHoldAccounts ihouseholdaccounts, INavigationService navigationService)
        {
            ...
            ResistCommand.Subscribe(_ =>
            {
                ...
                _householdaccounts.AddHouseHoldAccountsItem("test3", 1, 500, DateTime.Today, DCategorys.子供関連, SCategorys.日用雑貨, StorageTypes.財布, true);
                ...
                _householdaccounts.SetSCategoryStatics(SelectedRange.Value.R, (BalanceTypes)Enum.Parse(typeof(BalanceTypes), CurrentBalanceType), SelectedDate.Value, (SCategorys)Enum.Parse(typeof(SCategorys), CurrentSCategory));
                _householdaccounts.SetSCategoryStatisticsPie(SelectedRange.Value.R, SelectedDate.Value, (SCategorys)Enum.Parse(typeof(SCategorys), CurrentSCategory));
            }).AddTo(disposable);
        ...
        public void OnNavigatingTo(NavigationParameters parameters)
        {
            if (parameters.ContainsKey(InputKey))
            {
                ...
                this.CurrentBalanceType = NavigatedItem.SCItem.Balancetype;
                ...
                this.CurrentSCategory = NavigatedItem. SCItem.Scategory;
                this.SCategoryTitle = String.Format("家計簿・{0}", NavigatedItem.SCItem.Scategory);
                _householdaccounts.SetSCategoryStatics(SelectedRange.Value.R, (BalanceTypes)Enum.Parse(typeof(BalanceTypes),CurrentBalanceType), SelectedDate.Value, (SCategorys)Enum.Parse(typeof(SCategorys),CurrentSCategory));
                _householdaccounts.SetSCategoryStatisticsPie(SelectedRange.Value.R, SelectedDate.Value, (SCategorys)Enum.Parse(typeof(SCategorys), CurrentSCategory));
                this.DisplayScategoryTotal = _householdaccounts.ObserveProperty(h => h.SCategoryTotal).ToReactiveProperty().AddTo(disposable);
